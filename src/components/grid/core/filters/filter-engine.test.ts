...
// filter-engine.ts does real date math (isValid, startOf, isSame, isBefore,
// isAfter). The global dayjs mock in src/test/setup.ts answers every call
// with one fixed value and lacks isValid()/granularity comparisons, so opt
// this file back into the real library (same pattern as column-types.test.ts).
vi.unmock('dayjs')
