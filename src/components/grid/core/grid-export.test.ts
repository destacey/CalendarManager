import { describe, it, expect, vi, beforeEach } from 'vitest'

vi.unmock('dayjs')

vi.mock('../../../api/files', () => ({ saveFile: vi.fn() }))

import { saveFile } from '../../../api/files'
import { exportGridToCsv } from './grid-export'
import { buildHeadlessTable } from './test-table'
import { stringContainsFilter } from './grid-filters'

const decode = (bytes: Uint8Array) => new TextDecoder().decode(bytes)

describe('exportGridToCsv', () => {
  beforeEach(() => {
    vi.mocked(saveFile).mockReset()
    vi.mocked(saveFile).mockResolvedValue(true)
  })

  it('writes through the native save dialog, not a browser download', async () => {
    // WebView2 ignores a Blob <a download> entirely, so the only correct
    // path is the backend writing to a user-picked path.
    const table = buildHeadlessTable(
      [{ name: 'Ada', hours: 8 }],
      [
        { accessorKey: 'name', header: 'Name' },
        { accessorKey: 'hours', header: 'Hours' },
      ]
    )

    const written = await exportGridToCsv(table, 'timecards')

    expect(written).toBe(true)
    expect(saveFile).toHaveBeenCalledTimes(1)
    const [name, bytes, filterName, extensions] = vi.mocked(saveFile).mock.calls[0]
    expect(name).toMatch(/^timecards-\d{4}-\d{2}-\d{2}\.csv$/)
    expect(filterName).toBe('CSV')
    expect(extensions).toEqual(['csv'])
    expect(decode(bytes)).toContain('Name,Hours')
    expect(decode(bytes)).toContain('Ada,8')
  })

  it('prefixes a UTF-8 BOM so Excel does not mojibake multibyte text', async () => {
    const table = buildHeadlessTable(
      [{ name: 'Café 🚀' }],
      [{ accessorKey: 'name', header: 'Name' }]
    )

    await exportGridToCsv(table, 'events')

    const bytes = vi.mocked(saveFile).mock.calls[0][1]
    // EF BB BF
    expect(Array.from(bytes.slice(0, 3))).toEqual([0xef, 0xbb, 0xbf])
    expect(decode(bytes)).toContain('Café 🚀')
  })

  it('reports a cancelled dialog without throwing', async () => {
    vi.mocked(saveFile).mockResolvedValue(false)
    const table = buildHeadlessTable(
      [{ name: 'Ada' }],
      [{ accessorKey: 'name', header: 'Name' }]
    )

    await expect(exportGridToCsv(table, 'events')).resolves.toBe(false)
  })

  it('honours meta.enableExport, exportHeader and exportFormatter', async () => {
    const table = buildHeadlessTable(
      [{ name: 'Ada', secret: 'x', hours: 8 }],
      [
        { accessorKey: 'name', header: 'Name', meta: { exportHeader: 'Person' } },
        { accessorKey: 'secret', header: 'Secret', meta: { enableExport: false } },
        {
          accessorKey: 'hours',
          header: 'Hours',
          meta: { exportFormatter: (v: unknown) => `${v}h` },
        },
      ]
    )

    await exportGridToCsv(table, 'events')

    const csv = decode(vi.mocked(saveFile).mock.calls[0][1])
    expect(csv).toContain('Person,Hours')
    expect(csv).not.toContain('Secret')
    expect(csv).toContain('Ada,8h')
  })

  it('exports only the filtered rows, in the sorted order — not just membership', async () => {
    // Bob and Cara don't match the "ad" filter and must not appear at all.
    // Of the two that survive, sorting by hours (desc) puts Ada (8) before
    // Adam (5) — the opposite of their alphabetical/insertion order — so a
    // test that only checked membership would pass even if the export read
    // from the unsorted core row model.
    const table = buildHeadlessTable(
      [
        { name: 'Bob', hours: 3 },
        { name: 'Ada', hours: 8 },
        { name: 'Cara', hours: 1 },
        { name: 'Adam', hours: 5 },
      ],
      [
        { accessorKey: 'name', header: 'Name', filterFn: stringContainsFilter },
        { accessorKey: 'hours', header: 'Hours' },
      ],
      {
        columnFilters: [{ id: 'name', value: 'ad' }],
        sorting: [{ id: 'hours', desc: true }],
      }
    )

    await exportGridToCsv(table, 'events')

    const csv = decode(vi.mocked(saveFile).mock.calls[0][1])
    expect(csv).not.toContain('Bob')
    expect(csv).not.toContain('Cara')
    const adaIndex = csv.indexOf('Ada,8')
    const adamIndex = csv.indexOf('Adam,5')
    expect(adaIndex).toBeGreaterThan(-1)
    expect(adamIndex).toBeGreaterThan(-1)
    expect(adaIndex).toBeLessThan(adamIndex)
  })

  it('exports hidden columns as absent and reordered columns in display order', async () => {
    const table = buildHeadlessTable(
      [{ a: 'A1', b: 'B1', c: 'C1' }],
      [
        { accessorKey: 'a', header: 'A' },
        { accessorKey: 'b', header: 'B' },
        { accessorKey: 'c', header: 'C' },
      ],
      {
        columnVisibility: { b: false },
        columnOrder: ['c', 'a', 'b'],
      }
    )

    await exportGridToCsv(table, 'events')

    const csv = decode(vi.mocked(saveFile).mock.calls[0][1])
    const lines = csv.split('\n') // TextDecoder already strips a leading BOM
    expect(lines[0]).toBe('C,A')
    expect(lines[1]).toBe('C1,A1')
    expect(csv).not.toContain('B1')
  })

  it('writes a grouped-header prelude row with the label at the span start and blanks after', async () => {
    const table = buildHeadlessTable(
      [{ first: 'Ada', last: 'Lovelace', hours: 8 }],
      [
        {
          header: 'Person',
          columns: [
            { accessorKey: 'first', header: 'First' },
            { accessorKey: 'last', header: 'Last' },
          ],
        },
        { accessorKey: 'hours', header: 'Hours' },
      ]
    )

    await exportGridToCsv(table, 'events')

    const csv = decode(vi.mocked(saveFile).mock.calls[0][1])
    const lines = csv.split('\n') // TextDecoder already strips a leading BOM
    expect(lines[0]).toBe('Person,,')
    expect(lines[1]).toBe('First,Last,Hours')
    expect(lines[2]).toBe('Ada,Lovelace,8')
  })
})
