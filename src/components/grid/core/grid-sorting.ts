...
/**
 * Default sort for this grid that keeps **empty** values (null/undefined/'') at
 * the *end* of an ascending sort — and thus the start of a descending sort,
 * since TanStack negates the result on `desc`. This treats "empty" as the
 * largest value (like `+Infinity`), matching AG Grid / Excel, and fixes
 * TanStack's built-in behavior where a `null` number sorts as `0` (top of
 * ascending).
 *
