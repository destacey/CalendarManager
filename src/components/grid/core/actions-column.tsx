  ...
  return (
    <Dropdown menu={{ items }} trigger={['click']}>
      <Button
        ...
        icon={<MoreOutlined />}
        // Opts the button out of row activation — an activatable row must open
        // the dropdown, not also activate itself. See grid-row.tsx's
        // isInteractiveTarget (the `button` tag alone already covers a direct
        // click, but this makes the opt-out explicit and independent of tag).
...
/**
 * Builds the reusable row-actions column for a DataGrid table. Everything
 * about the column is standardized — a fixed-width, non-sortable/-filterable/
 * -resizable `⋯` dropdown that hides itself when a row has no items — except
 * {@link ActionsColumnOptions.getItems}, which each grid supplies to compute
 * per-row menu items (typically gated on permissions).
 *
 * @example
...
export const createActionsColumn = <T extends RowData,>({
  ...
  cell: ({ row }: { row: Row<T> }) =>
    leading === undefined ? (
      ...
      <Flex align="center" gap={2} data-row-activate="ignore">
        {leading}
        <ActionsCell row={row.original} getItems={getItems} ariaLabel={ariaLabel} />
      </Flex>
    ),
