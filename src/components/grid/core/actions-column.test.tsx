...
describe('createActionsColumn', () => {
  ...
  describe('cell rendering', () => {
    it('calls getItems with the row original', () => {
      // Arrange
      const getItems = vi.fn<(row: Widget) => ItemType[]>(() => [
        { key: 'edit', label: 'Edit' },
      ])
      const col = createActionsColumn<Widget>({ getItems })
    ...
    it('marks the trigger button so it does not also activate an activatable row', () => {
      // Arrange
      const col = createActionsColumn<Widget>({
        getItems: () => [{ key: 'edit', label: 'Edit' }],
      })
      ...
      // Assert — see grid-row.tsx's isInteractiveTarget escape hatch
