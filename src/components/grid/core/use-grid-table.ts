...
import { dataGridFeatures } from './grid-features'
...
export interface UseGridTableOptions<T extends RowData> {
   ...
   * (defaultColumn, initialState, …). Cannot override the state/handler
   * wiring, which the hook owns. Row models are not per-table options in v9 —
   * they are registered in grid-features.ts. Row expanding and row selection
   * are not registered features for this grid, so their options (e.g.
   * getSubRows) have no effect here even if passed.
   */
  tableOptions?: Omit<
...
/**
 * Creates the TanStack table with this grid's shared configuration: filtered
