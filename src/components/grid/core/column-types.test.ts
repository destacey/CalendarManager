...
// column-types.ts formats real dates with dayjs; the fixed-value dayjs mock
// `src/test/setup.ts` installs globally would make every formatted date
// assert as the same string, so opt this file back into the real library
// (same pattern as MapEvents.test.tsx / calendar.test.ts).
vi.unmock('dayjs')
...
describe('applyColumnType', () => {
  ...
  describe('yesNo', () => {
    ...
    it('sets a set filter offering Yes / No, not true / false', () => {
      // Arrange / Act / Assert
      const meta = resolved.meta as DataGridColumnMeta
    ...
    it('uses a set filter (Excel-style checkbox list of False/True)', () => {
      ...
      // single-select mode.
      expect((resolved.meta as DataGridColumnMeta).filterType).toBe('set')
    })
  })
  ...
  describe('dateOnly / dateTime', () => {
    it('keeps the raw value as the accessor (for sort/filter) — no transform', () => {
      ...
      expect((resolved.meta as DataGridColumnMeta).filterType).toBe('date')
    })
    ...
    it('uses the dateTime filter type for dateTime columns', () => {
      ...
      // Assert
      expect((resolved.meta as DataGridColumnMeta).filterType).toBe('dateTime')
    })
  ...
  describe('precedence', () => {
    it('does not override an explicit filterType on the column', () => {
      ...
      // Assert
      expect((resolved.meta as DataGridColumnMeta).filterType).toBe('text')
    })
