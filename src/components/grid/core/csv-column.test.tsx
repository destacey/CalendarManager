...
import { SET_FILTER_BLANK } from './filters'
...
describe('createCsvColumn', () => {
  ...
  it('matches a blank (no owners) row only via the (Blanks) sentinel', () => {
    // Arrange — a consumer's set filter panel offers a "(Blanks)" entry for
    // rows with no tokens at all; createCsvColumn's wired filterFn must honor
    // it the same way the underlying engine does.
    const filterFn = column.filterFn as (
      row: Row<Project>,
      columnId: string,
      value: unknown,
    ) => boolean
    const blankModel: SetFilterModel = { type: 'set', values: [SET_FILTER_BLANK] }

    // Act / Assert
    expect(filterFn(rowOf({ owners: '' }), 'owners', blankModel)).toBe(true)
    expect(filterFn(rowOf({ owners: 'Bob' }), 'owners', blankModel)).toBe(
      false,
    )
  })
