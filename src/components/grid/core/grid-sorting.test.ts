...
// dateSortBy does real date math (dayjs(...).valueOf()). The global dayjs
// mock in src/test/setup.ts answers every call with one fixed value
// (valueOf() always 1704067200000), which would make every date compare
// equal — opt this file back into the real library (same pattern as
// filter-engine.test.ts / column-types.test.ts).
vi.unmock('dayjs')
